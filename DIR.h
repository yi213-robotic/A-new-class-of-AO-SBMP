...
inline double trajTime(ompl::base::State *src_,  ompl::base::State *tar_) 
{
...
inline ompl::base::Cost trajCost(ompl::base::State *src_,  ompl::base::State *tar_, double arriveTime_)
{
      const auto *src10D = src_->as<ob::RealVectorStateSpace::StateType>(); 
      const auto *tar10D = tar_->as<ob::RealVectorStateSpace::StateType>();
...
inline void steeringDDI(double t, double t_s, double x01, double x02, double x03, double x04, double x11, double x12, double x13, double x14, bool pathN, ompl::base::State *state)
{
             ...
             if(pathN)
             std::cout <<" " << rstate->values[0] << " " << rstate->values[1] << " " << rstate->values[2] << " " << rstate->values[3] << std::endl;     
}

inline ompl::base::Cost motionCost(ompl::base::State *src_,  ompl::base::State *tar_)
{
     double arriveTime_ = trajTime(src_,tar_);
     return trajCost(src_,tar_,arriveTime_);
}
